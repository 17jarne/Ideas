    ...
    def birth(self, commandtime):                                            #time between requests for new commands
        ...
        while commandtemp > -1:
            ...
            for neuron in self.neurons:                                             #signal processing
                ...
                situimportance = situimportance*0.5
            for neuron in self.amygdala_c.neurons:
                if neuron.status == 1:
                    situimportance = situimportance + 1
            for neuron in self.neurons:
                neuron.activation(situimportance)
            for neuron in self.neurons:
                neuron.resetweights()
                print neuron.status
            output = []
            ...
            print output
            if 'sensory0' in impression and self.outneurons.__getitem__(0).status == 1:
                neuron = self.amygdala_c.neurons.__getitem__(0)
                neuron.sumincomings(neuron.actthreshold)
            if 'sensory1' in impression and self.outneurons.__getitem__(0).status == 1:
                neuron = self.amygdala_c.neurons.__getitem__(self.amygdala_c.joysize) 
                neuron.sumincomings(neuron.actthreshold)
            commandtemp += 1
